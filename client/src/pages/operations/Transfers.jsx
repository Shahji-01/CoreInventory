...
import { getTransfers, createTransfer, updateTransfer, deleteTransfer, validateTransfer, cancelTransfer } from '../../services/transferService';
import { getProducts } from '../../services/productService';
import { getWarehouses } from '../../services/warehouseService';
import { Plus, CheckCircle, Shuffle, Trash2, X, XCircle, Edit2, Eye } from 'lucide-react';
...
export default function Transfers() {
  ...
  const [form, setForm] = useState({ fromWarehouseId: '', toWarehouseId: '', notes: '', lines: [] });
  ...
  const submit = async (e) => {
    ...
    try {
      if (editId) {
        ...
        toast.success('Transfer updated', `Transfer has been updated.`);
      } else {
        await createTransfer(form);
        toast.success('Transfer created', `Transfer has been added.`);
      }
      setOpen(false); setEditId(null);
      setForm({ fromWarehouseId: '', toWarehouseId: '', notes: '', lines: [] });
      load();
    } catch (ex) { 
      const msg = ex.message || `Failed to ${editId ? 'update' : 'create'} transfer`;
      setErr(msg);
  ...
  const validate = async (id, e) => {
    ...
    if (!confirm('Validate this transfer? Stock will be moved instantly.')) return;
    try { 
      await validateTransfer(id); 
      toast.success('Transfer validated', 'Stock has been transferred.');
  ...
  const cancelOp = async (id, e) => {
    ...
    try { 
      ...
      toast.success('Transfer cancelled', 'The operation was cancelled.');
  ...
  const deleteOp = async (id, e) => {
    ...
    try {
      ...
      toast.success('Transfer deleted', 'The draft transfer has been removed.');
      if (viewItem && viewItem.id === id) setViewItem(null);
      load();
    } catch (ex) { toast.error('Deletion failed', ex.message || 'Failed to delete transfer'); }
  };
  ...
  const openEdit = (r, e) => {
    if (e) e.stopPropagation();
    setForm({
      fromWarehouseId: r.fromWarehouseId || r.fromWarehouse?._id || '',
      toWarehouseId: r.toWarehouseId || r.toWarehouse?._id || '',
      notes: r.notes || '',
      lines: r.lines.map(l => ({ 
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-header-title">Transfers</div>
          <div className="page-header-sub">Move stock between warehouses</div>
        </div>
      ...
      <div className="card table-card">
        {loading ? <div className="loading-overlay"><div className="spinner" /></div> : filtered.length === 0 ? (
          <div className="empty-state">
            <Shuffle className="empty-state-icon" style={{ display: 'inline' }} />
            <div className="empty-state-title">No transfers found</div>
            <div className="empty-state-text">{statusFilter ? 'No transfers with this status.' : 'Create your first transfer to move stock.'}</div>
          </div>
        ) : (
          <table>
            <thead>
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.id} onClick={() => setViewItem(r)} style={{ cursor: 'pointer' }} className="hover-row">
                  ...
                  <td><span className="text-muted">{r.fromWarehouseName}</span></td>
                  <td><span className="text-muted">{r.toWarehouseName}</span></td>
        ...
        <div className="modal-overlay" onClick={() => setOpen(false)}>
          <div className="slide-over" onClick={e => e.stopPropagation()}>
            <div className="slide-over-header">
              <span className="slide-over-title">{editId ? 'Edit Transfer' : 'New Transfer'}</span>
            ...
            <form onSubmit={submit} style={{ display: 'contents' }}>
              <div className="slide-over-body">
                ...
                <div className="form-group">
                  ...
                  <select className="form-input" value={form.fromWarehouseId} onChange={e => setForm({...form, fromWarehouseId: e.target.value})} required>
                ...
                <div className="form-group">
                  ...
                  <select className="form-input" value={form.toWarehouseId} onChange={e => setForm({...form, toWarehouseId: e.target.value})} required>
                    <option value="">Select Warehouse...</option>
                    {reqs.warehouses.map(w => <option key={w.id || w._id} value={w.id || w._id}>{w.name}</option>)}
                  </select>
                </div>
                          ...
                          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: 'var(--bg-elevated)', padding: 10, borderRadius: 8, border: '1px solid var(--border)' }}>
                            <div style={{ flex: 1 }}>
                              <select className="form-input" value={ln.productId} onChange={e => updateLine(i, 'productId', e.target.value)} required>
                                ...
                                {reqs.products.map(p => <option key={p.id || p._id} value={p.id || p._id}>{p.name}</option>)}
                              </select>
                            </div>
                            ...
                            <button type="button" className="btn-icon" style={{ color: 'var(--rose)', background: 'transparent', border: 'none', cursor: 'pointer', padding: '10px 4px' }} onClick={() => removeLine(i)}><Trash2 size={14} /></button>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div className="slide-over-footer">
                ...
                <button type="submit" className="btn btn-primary" style={{ flex: 1, justifyContent: 'center' }} disabled={saving || form.lines.length === 0}>
                  {saving ? <span className="spinner" /> : (editId ? 'Save Changes' : 'Create Transfer')}
                </button>
              </div>
            </form>
          </div>
            ...
            <div className="slide-over-body" style={{ padding: 24 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
                <div>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{viewItem.fromWarehouseName}</div>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>Destination Warehouse</div>
                  <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{viewItem.toWarehouseName}</div>
                </div>
